   ...
   client.query(selquery, function(err, result, fields) {
     ...
     if (result.rowCount > 0)
     {
      var data = "";
       ...
       {
         tmpval = result.rows[i].timestamp+";"+result.rows[i].symbol+";"+result.rows[i].sharestraded+";"+result.rows[i].pricetraded+";"+result.rows[i].changedirection+";"+result.rows[i].changeamount;
         if (i == 0)
         {
           data = tmpval;
         }
